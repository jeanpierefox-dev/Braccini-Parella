...
export const Login: React.FC<LoginProps> = ({ onLogin, users, isCloudConnected, onOpenCloudConfig }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-corp-bg relative overflow-hidden p-6">
      ...
      <button onClick={onOpenCloudConfig} className="absolute top-6 right-6 text-slate-500 hover:text-corp-accent transition z-20" title="Configuración de Nube">
      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="bg-corp-panel/50 backdrop-blur-xl border border-white/5 rounded-2xl shadow-2xl p-8">
            ...
            <div className="space-y-1.5">
                ...
                <input type="text" required value={username} onChange={(e) => setUsername(e.target.value)} className="w-full px-4 py-3 bg-black/20 border border-white/10 rounded-lg text-white text-sm focus:border-corp-accent focus:bg-black/40 focus:ring-1 focus:ring-corp-accent outline-none transition-all placeholder-slate-600" placeholder="Ingrese su ID" />
            </div>
            <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Contraseña</label>
                <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 bg-black/20 border border-white/10 rounded-lg text-white text-sm focus:border-corp-accent focus:bg-black/40 focus:ring-1 focus:ring-corp-accent outline-none transition-all placeholder-slate-600" placeholder="••••••••" />
            </div>
            {error && <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-3 rounded-lg text-xs font-medium text-center">{error}</div>}
            <button type="submit" className="w-full bg-corp-accent hover:bg-corp-accent-hover text-white font-bold py-3.5 rounded-lg shadow-lg shadow-blue-500/20 transition-all transform active:scale-[0.98] mt-2 text-sm uppercase tracking-wide" disabled={!username || !password}>Iniciar Sesión</button>
            </form>
             <div className="mt-8 pt-4 border-t border-white/5 flex justify-between items-center text-xs text-slate-500">
                 <div className="flex items-center gap-2"><div className={`w-1.5 h-1.5 rounded-full ${isCloudConnected ? 'bg-green-500' : 'bg-slate-600'}`}></div><span>{isCloudConnected ? 'Cloud Sync On' : 'Modo Local'}</span></div>
