...
export const Layout: React.FC<LayoutProps> = ({ 
  children, currentUser, onLogout, onNavigate, currentView, isCloudConnected, onOpenCloudConfig
}) => {
  ...
  const NavItem = ({ view, label, icon, isMobile = false }: { view: string, label: string, icon: React.ReactNode, isMobile?: boolean }) => {
    ...
    if (isMobile) {
        return (
            <button onClick={() => onNavigate(view)} className={`flex-1 flex flex-col items-center justify-center py-3 transition-all duration-300 relative ${isActive ? 'text-white' : 'text-slate-400'}`}>
    ...
    return (
        <button onClick={() => onNavigate(view)} className={`relative flex items-center gap-2 px-4 py-2 rounded-md transition-all font-medium text-sm ${isActive ? 'bg-corp-accent text-white shadow-md' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
      ...
      <header className="bg-corp-panel/80 border-b border-white/5 sticky top-0 z-50 backdrop-blur-md h-16">
        <div className="max-w-7xl mx-auto px-6 h-full flex justify-between items-center">
            ...
            <div className="flex items-center gap-4 ml-auto">
               {isAdmin && (
                   <button onClick={onOpenCloudConfig} className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-[10px] font-bold uppercase tracking-widest transition ${isCloudConnected ? 'border-green-500/20 bg-green-500/10 text-green-400' : 'border-red-500/20 bg-red-500/10 text-red-400'}`}>
               ...
               <div className="flex items-center gap-3 pl-4 border-l border-white/5">
                   ...
                   <button onClick={onLogout} className="text-slate-500 hover:text-white transition ml-2" title="Cerrar Sesión">{Icons.Logout}</button>
               </div>
            </div>
        </div>
      </header>
